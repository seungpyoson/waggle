...
// TestLoadAgentConfig_Default — missing file reads defaults without writing configuration with claude/codex/gemini
func TestLoadAgentConfig_Default(t *testing.T) {
	...
	if err != nil || len(entries) != 0 {
		t.Fatal("configuration read created state", err)
	}
	if len(cfg.Agents["claude"].Args) != 0 {
		t.Fatal("interactive launch default enables headless mode")
	}
}
...
func TestGetAgent_Specific(t *testing.T) {
	...
	_, agent, err := cfg.GetAgent("codex")
	if err != nil {
