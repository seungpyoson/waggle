...
type MessagingConfig struct {
	...
	IdleCheckInterval  time.Duration // per-worker periodic queue check without a wake
...
func NewMessagingConfig() MessagingConfig {
	return MessagingConfig{
		...
		DiscoveryInterval: 500 * time.Millisecond, QueueCheckInterval: 50 * time.Millisecond,
		IdleCheckInterval: 2 * time.Second, ProbeInterval: 5 * time.Second, ReconnectBackoff: time.Second,
...
func (c MessagingConfig) Validate() error {
	...
	if c.DiscoveryInterval <= 0 || c.QueueCheckInterval <= 0 || c.IdleCheckInterval <= 0 || c.ProbeInterval <= 0 || c.ReconnectBackoff <= 0 {
		return fmt.Errorf("enrollment worker intervals must be positive")
	}
	if c.QueueCheckInterval > c.IdleCheckInterval {
		return fmt.Errorf("queue-check floor must not exceed the idle-check interval")
	}
